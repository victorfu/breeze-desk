...
電腦,唯一可能的網路活動是經過驗證的模型下載,以及選擇啟用的更新檢查。開始轉錄時若缺少
必要的內建 Silero VAD 模型,程式會自動下載固定版本並完成驗證。
...
自訂模型。需要 Silero VAD 的佇列工作若發現模型缺少或無法通過驗證,會走相同的安全下載
流程,安裝完成後自動從原階段繼續。
...
一般 Windows 建置預設使用 CPU 版 whisper.cpp Worker，因此不需要額外安裝 Vulkan
